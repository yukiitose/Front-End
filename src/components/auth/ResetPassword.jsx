...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.password || !form.password_confirmation) { setError('Please fill in all fields.'); return }
    ...
    try {
      const res = await axios.post('http://127.0.0.1:8000/api/reset-password',
        { token, email, password: form.password, password_confirmation: form.password_confirmation },
    ...
    <div style={{minHeight:'100vh',background:'#f0f0f0',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{background:'#fff',borderRadius:12,padding:'48px 40px',width:'100%',maxWidth:420,boxShadow:'0 4px 24px rgba(0,0,0,0.10)'}}>
        ...
        <div style={{textAlign:'center',marginBottom:28}}>
          ...
          <h2 style={{color:'#B71C1C',fontWeight:800,fontSize:22,margin:0}}>Reset Password</h2>
        ...
        {message ? (
          <div style={{textAlign:'center'}}>
            ...
            <h3 style={{color:'#2e7d32',fontWeight:700,margin:'0 0 8px'}}>Password Reset!</h3>
            <p style={{color:'#555',fontSize:14}}>{message}</p>
            <p style={{color:'#888',fontSize:13,marginTop:8}}>Redirecting to login in 3 seconds...</p>
            <Link to="/login" style={{display:'inline-block',marginTop:16,color:'#B71C1C',fontWeight:700,fontSize:14}}>Go to Login →</Link>
          </div>
        ) : (
          <>
